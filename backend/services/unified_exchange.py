...
class UnifiedExchangeService:
    ...
    async def get_balance(
        ...
        """
        # ✅ FIX: Convert exchange to lowercase once and store
    ...
    async def get_current_price(
        ...
        """
        # ✅ FIX: Convert exchange to lowercase once
        exchange_name = str(exchange).lower()
        ...
        try:
            ...
            logger.error(f"Price fetch failed for {exchange_name} {symbol}: {str(e)}")
            raise ExchangeError(exchange_name, f"Failed to fetch price for {symbol}: {str(e)}", e)
    ...
    async def get_positions(
        ...
        try:
            ...
            logger.error(f"Positions fetch failed for {exchange_name}: {str(e)}")
            raise ExchangeError(exchange_name, f"Failed to fetch positions: {str(e)}", e)
